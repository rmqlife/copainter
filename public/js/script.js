...
const sketch = function(p) {
  ...
  */
  ...
  function getData() {
    ...
    //} else {
      $.get("/getData", function(data) {
        ...
        document.getElementById("green-cursor").style.left = data["x"] + "px";
        document.getElementById("green-cursor").style.top = (data["y"] + 144) + "px";
        mouseX = data["x"];
        mouseY = data["y"];
        //console.log(document.getElementById("green-cursor").style.left, data["x"],document.getElementById("green-cursor").style.top, data["y"]);
        prevPressed = curPressed;
        curPressed = data["pressed"];
        if (prevPressed === 0 && curPressed === 1) {
          mousePressed();
        } else if (prevPressed === 1 && curPressed === 1) {
          mouseDragged();
        } else if (prevPressed === 1 && curPressed === 0) {
          mouseReleased();
        }
        if (data["jstick"] >= 0 && data["jstick"] < 128) {
          p.updateCurrentColor(randomColorIndex());
        } else if (data['jstick'] >= 128 && data['jstick'] <= 255) {
          console.log("change model");
          initModel(Math.floor(Math.random() * availableModels.length));
        }
        if (curPressed === 1 && !isInBounds()) {
  ...
  function isInBounds() {
      ...
      return mouseX >= 0 && mouseY >= 0 && mouseX < p.width && mouseY < p.height;
  }
  ...
  */
  ...
  p.updateCurrentColor = function(index) {
    ...
    console.log("storke color change: " + currentColor);
  }
